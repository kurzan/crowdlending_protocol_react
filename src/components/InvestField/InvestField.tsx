...
const InvestField: FC<TInvestFieldProps> = ({id, currentBorrow}) => {
  ...
  return (
    <form action="" className={styles.form}>
      ...
      {isConnected && <Button onClick={() => write?.()} disabled={prepareLoading || isLoadingInvestData || error ? true : false} title={!isLoadingInvestData ? "Инвестировать" : "Подвтердите действие..."} />}
      {error ? (
        <Box margin="0" >
